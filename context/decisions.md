...
**Decision**: Migrer OpenClaw sur un VPS Hetzner CX23 (2 vCPU, 4GB RAM, 40GB SSD, cost-optimized — https://www.hetzner.com/cloud/cost-optimized). Domaine enregistré : `elibla.com`. Page racine à la racine, sous-domaines pour les installs/apps (OpenClaw, micro-app, etc.). Déploiement prévu le 2026-03-30 au soir.
...
**Consequences**: OpenClaw tourne 24/7 indépendamment du Mac d'Élie. Le briefing quotidien devient fiable. Le repo `openclaw-elie/` devra être adapté pour refléter le déploiement distant. `elibla.com` servira aussi de vitrine/portfolio pour les apps d'Élie.
